	...
	default:
		...
		case '/': // Prefix: "/documents"
			...
			case '/': // Prefix: "/"
				...
				switch elem[0] {
				case 'b': // Prefix: "by-number/"
					origElem := elem
					if l := len("by-number/"); len(elem) >= l && elem[0:l] == "by-number/" {
						elem = elem[l:]
					} else {
						break
					}
					...
					if len(elem) == 0 {
						...
						switch r.Method {
						case "DELETE":
							s.handleDeleteDocumentByNumberRequest([1]string{
								args[0],
							}, elemIsEscaped, w, r)
						default:
							s.notAllowed(w, r, notAllowedParams{
		...
		case '/': // Prefix: "/documents"
			...
			if len(elem) == 0 {
				...
				case "POST":
					...
					r.summary = "Save parsed document"
			...
			case '/': // Prefix: "/"
				...
				case 'b': // Prefix: "by-number/"
					...
					if len(elem) == 0 {
						...
						switch method {
						case "DELETE":
							r.name = DeleteDocumentByNumberOperation
							r.summary = "Delete document by number"
							r.operationID = "deleteDocumentByNumber"
							r.operationGroup = ""
							r.pathPattern = "/documents/by-number/{document_number}"
							r.args = args
							r.count = 1
							return r, true
						default:
